import { getPercentageStrColor, getLabelForMasterCSV } from "../../helpers/numerals"
...
export const downloadMasterExcel = (projects: ProjectsType) => {
  ...
  // Twitter
  const twitterTitle = worksheet.getCell("BB2")
        ...
        row.values.forEach((value, index) => {
          ...
          if (typeof value === "string" && value.endsWith("%")) {
            ...
            row.getCell(index).font = {
              color: { argb: getPercentageStrColor(numVal) },
            }
          ...
          } else if (typeof value === "string" && value.endsWith("PS_RATIO")) {
            const numVal = value.split("PS_RATIO")[0]
            if (!!Number(numVal)) {
              row.getCell(index).value = Number(numVal)
